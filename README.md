...
大沼チームの月次共有事項（近況・困りごと、グループワーク振り返り、稼働着地、面談希望、総評）をSlackのモーダルUIからスプレッドシートへ自動入力し、未入力者への自動リマインド、さらに **Googleプラットフォーム（Cloud Run + Google AI Studio）の完全無料枠** で運用可能なSlack Botです。
